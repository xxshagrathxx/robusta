                            ...
                            <form action="{{ route('trips-store') }}" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="from_city_id" id="from_city_id" class="form-select">
                                            ...
                                            @foreach ($stations as $station)
                                                <option value="{{ $station->id }}" {{ $station->id == $trip->from_city_id ? 'selected' : '' }}>{{ $station->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('from_city_id')
                                ...
                                <div class="col-6 mb-3">
                                    <div class="form-group">
                                        ...
                                        <select name="to_city_id" id="to_city_id" class="form-select">
                                            <option value="" selected="" disabled="">{{ transWord('Select City') }}</option>
                                            @foreach ($stations as $station)
                                                <option value="{{ $station->id }}"  {{ $station->id == $trip->to_city_id ? 'selected' : '' }}>{{ $station->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('to_city_id')
                                            <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="col-6 mb-3">
                                    <div class="form-group">
                                        ...
                                        <textarea rows="3" class="form-control" name="via_city_ids" placeholder="Enter Via Cities">{{ old('via_city_ids', $trip->via_city_ids) }}</textarea>
